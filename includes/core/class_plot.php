...
class Plot {
    ...
    public static function plots_ids($d = []) {
        $q = DB::query("SELECT plot_id FROM plots;") or die (DB::error());
        while ($row = DB::fetch_row($q)) {
            $items[] = (int) $row['plot_id'];
        }
        if($d) {
            $exists_plots = array_intersect($items,$d);
            return $exists_plots ? $exists_plots : [''];
        }
