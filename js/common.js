let common = {
    ...
    search_do: (act) => {
        // vars
        let data = { search: gv('search'), selector: gv('search-selector') };
    ...
    user_delete_window: (user_id, e) => {
        ...
        <div class="modal_head">
            <div>Confirmation of deletion</div>
        ...
        <div class="modal_body">
            ...
            <div class="modal_controls modal_controls_small">
                <div>
                    <div class="btn_modal" onclick="common.user_edit_delete(${user_id});">Delete</div>
                </div>
                <div>
    ...
    plot_input: () => {
        let value = gv("plot_id");
        Array.prototype.slice.call( ge('plot__suggest').children, 0 )
        .forEach((item)=>{
            let lplot = value.replace(" ", '').split(',');
            if (!item.textContent.includes(lplot.pop()))
                item.classList.add('hidden_item')
            else item.classList.remove('hidden_item')
        });
    },

    close_select: () => {
        setTimeout(() => {document.getElementById('plot_select').classList.remove('active')}, 200)
    },
    ...
    user_edit_update: (user_id = 0) => {
        ...
        let data = {
            user_id: user_id,
            phone: (ph = gve('phone')).match("^([0-9])+$") ? ph : 0,
            first_name: gve('first_name'),
            last_name: gve('last_name'),
            email: gve('email').toLowerCase(),
            plot_id: gv('plot_id'),
            offset: global.offset
        };
        if(!document.querySelectorAll(".modal__input_error").length && data.phone != 0) {
